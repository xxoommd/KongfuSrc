...
function ViewBase:createMenu(normalSpritePath, selectedSpritePath, position, callback)
    local item = cc.MenuItemImage:create()
    item:setNormalSpriteFrame(display.newSpriteFrame(normalSpritePath))
    item:setSelectedSpriteFrame(display.newSpriteFrame(selectedSpritePath))
    item:registerScriptTapHandler(callback)
    local menu = cc.Menu:create(item)
    menu:move(position):addTo(self)
    return menu
end
